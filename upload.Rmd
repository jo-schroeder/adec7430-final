...
summary_df %>%
  kable(row.names = FALSE, col.names = c("", "Model 2 - Time Series Predictors Only")) %>%
  add_header_above(c("Random Forest Model " = 3)) %>%
...
accuracy_table %>%
  kable(row.names = FALSE, col.names = c("", "Model 2 - Time Series Predictors Only")) %>%
  add_header_above(c("Model Accuracy" = 2)) %>%
  kable_styling(full_width = FALSE)
```
...
metrics_df %>%
  kable(row.names = FALSE, col.names = c("", "Model 2 - Time Series Predictors Only")) %>%
  add_header_above(c("Evaluation Metrics" = 2)) %>%
  kable_styling(full_width = FALSE)
```
